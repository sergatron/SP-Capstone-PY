...
def remove_symbols(df):
  ...
  for item in cols:
    ...
    if item == 'rebate':
      df.loc[:, 'rebate'] = df.loc[:, 'rebate'].str.replace(',', '')
  return df
...
# plots
plt.hist(state_count, bins=70)
# without CA
plt.hist(state_count[state_count<646408], bins=80)
plt.hist(state_count[state_count<15000], bins=80)
plt.hist(state_count[state_count>2800], bins=80)
plt.show()
...
pv_df2 = pv_df2[['date_installed', 'city','state', 'full', 'county', 'size_kw','annual_insolation',
               'annual_PV_prod', 'reported_annual_energy_prod','cost_per_watt','cost','rebate',
...
# rename columns
pv_df.columns = ['date_installed', 'city', 'state_short', 'state', 'county', 'population',
                 'size_kw', 'annual_insolation', 'annual_pv_prod',
                 'reported_annual_energy_prod', 'cost_per_watt', 'cost', 'rebate',
...
missing_val_count(pv_pop, 'incentive_prog_names')
...
# read
pv_pop = load_csv('pv_pop_clean.csv')
pv_pop.info()
pv_pop.head()
...
state_count.describe()
state_count['california':'rhode island'].plot(kind='bar')

# COUNTY COUNT
county_count = pv_pop['county'].value_counts(dropna=False)
...
install_type_size.plot(kind='bar')
...
pv_pop[pv_pop['install_type']=='residential']
...
# explore DF where it's equal to the max
residential_sub[residential_sub['size_kw']==737319]

# explore DF where it's equal to the min
residential_sub[residential_sub['size_kw']==0.020000]

# sort size
size_sorted = residential_sub['size_kw'].sort_values()
residential_sub[size_sorted<1989]['size_kw'].describe()
size_sorted_1989 = residential_sub[size_sorted<1989]['size_kw']

# plot histogram after sorting
n_bins = int(np.sqrt(len(residential_sub[size_sorted<100]['size_kw'])))
residential_sub[size_sorted<100]['size_kw'].plot(kind='hist', bins=n_bins)
...
residential_sub['cost'].sort_values()
residential_sub['cost'].plot(kind='hist', bins=2000)
...
#plt.text(x=1750, y=4000000, s='Expensive!')
plt.show()

# drop NA before computing correlation
np.corrcoef(residential_sub['size_kw'],residential_sub['cost'])
...
residential_cost = residential_sub.groupby('state')[['rebate', 'cost']].median().sort_values(by='cost')
...
# COST
# =============================================================================
residential_cost = residential_sub.groupby('state')[['cost', 'after_rebate']].median().sort_values(by='after_rebate')
...
residential_rebate = residential_sub.groupby('state')['rebate'].median().sort_values()
residential_rebate['maryland':'florida'].plot(kind='bar')
plt.title('Median Rebate')
plt.ylabel('Rebate')
plt.xlabel('State')
plt.xticks(rotation=70)
#plt.tight_layout()
...
# =============================================================================
# cost over the years
pv_pop_date = pd.read_csv('pv_pop_clean.csv', sep=',', low_memory=False, index_col='date_installed', parse_dates=True, infer_datetime_format=True)
pv_pop_date = pv_pop_date.sort_index()

# subplot residential
residential_sub_2 = pv_pop_date[pv_pop_date['install_type']=='residential']
residential_sub_2['cost'].describe()

# resample yearly
yearly = pv_pop_date.resample('M').mean()
yearly = yearly.dropna()
yearly.info()
#yearly = yearly['1998-12-31':'2015-12-31']

yearly.plot(kind='scatter', y=yearly['cost'], x=yearly['date_installed'])
